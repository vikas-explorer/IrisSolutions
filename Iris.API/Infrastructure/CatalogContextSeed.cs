...
namespace Iris.API.Infrastructure
{
    public class CatalogContextSeed
    {
        public async Task SeedAsync(CatalogDbContext context, IHostingEnvironment env, ILogger<CatalogContextSeed> logger)
        {
            var policy = CreatePolicy(logger, nameof(CatalogDbContext));

            await policy.ExecuteAsync(async () =>
            {
                var useCustomizationData = true;// settings.Value.UseCustomizationData;
